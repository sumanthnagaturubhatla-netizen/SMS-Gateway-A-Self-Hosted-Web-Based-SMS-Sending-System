...
1. **Direct HTTP Server Mode:** Interacts directly with the "Simple SMS Gateway" app running a web server on the phone.
2. **Termux:API Client Mode:** Runs a lightweight background daemon on the Android device via Termux to poll the server for pending SMS and send them via `termux-sms-send`.
